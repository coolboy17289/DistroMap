"""
Single-file FastAPI for user-submitted distro suggestions, wrapped
with Mangum so it runs on the Vercel Python runtime as a single
serverless function.

Why this is a single file
-------------------------
Vercel's @vercel/python runtime auto-discovers every ``.py`` file
under the project root's ``api/`` directory and creates one function
per file. If we split the FastAPI app into ``api/index.py`` (Mangum
entry) + ``api/app.py`` (FastAPI defs), Vercel would also deploy
``api/app.py`` as a stub function at ``/api/app`` that 500s every
request. Keeping the FastAPI app and ``handler = Mangum(...)`` in the
same file avoids the stub-function footgun entirely. The
``vercel.json`` rewrite ``/api/(.*) → /api/index.py`` makes every
``/api/*`` request land on this single function, where Mangum
dispatches the ASGI scope into FastAPI's router.

Local dev: ``cd frontend && uvicorn api.index:app --reload --port 8765``
Production: Vercel loads ``handler = Mangum(app, lifespan="off")``
            with ``lifespan="off"`` because serverless cold starts
            shouldn't fire FastAPI startup/shutdown hooks.

Endpoints
---------
GET  /api/health         → { ok: True, suggestions: <int>, mode: ... }
GET  /api/suggestions    → [Suggestion, ...]                          (last 100)
POST /api/suggestions    → { ok: True, id: <slug-ts-rand> }            (append)

Storage
-------
KV mode (production): Upstash Redis REST via urllib (no httpx
import cost on cold start); atomic EVAL with a Lua script does
dup-check + append in one round-trip.
File mode (local dev): JSON at ``frontend/.cache/api/suggestions.json``
threading.Lock around read-modify-write so concurrent POSTs don't
lose rows. Vercel's prod filesystem is read-only so **file mode
will 500 on production POST without a linked KV store — link KV
from Vercel → Storage → Create Database → KV**.
"""
...
# ROOT is frontend/ because this file lives at frontend/api/index.py.
# parents[0] = frontend/api, parents[1] = frontend.
...
# Dual-route under both `/api/health` and `/health` because Vercel
# has historically had ambiguous behavior on whether the rewrite
# `/api/(.*) → /api/index.py` forwards the full path or strips the
# function-mount prefix before Mangum sees the ASGI scope. Cover
# both shapes; the handlers are intentionally identical so the wire
# contract matches whichever path Vercel uses.
@app.get("/api/health")
...
def health_no_prefix() -> dict[str, Any]:
    return _health_impl()
    # KV mode is atomic server-side; file mode needs the lock to
    # avoid reading a half-written JSON. Branching on mode keeps
    # file-mode concurrency race-tested without paying for an
...
def _list_suggestions_impl(limit: int) -> list[dict[str, Any]]:
...
# Dual-route (see note on /api/health above) — register the
# suggestion list endpoint under both shapes so Vercel's
# path-forwarding ambiguity can't 404 us.
@app.get("/api/suggestions")
def list_suggestions(limit: int = 100) -> list[dict[str, Any]]:
    return _list_suggestions_impl(limit)


@app.get("/suggestions", include_in_schema=False)
def list_suggestions_no_prefix(limit: int = 100) -> list[dict[str, Any]]:
    return _list_suggestions_impl(limit)
    if limit < 1 or limit > 500:
        raise HTTPException(status_code=400, detail="limit must be 1..500")
    if _using_kv():
...
def _post_suggestion_impl(payload: SuggestionIn) -> dict[str, Any]:
    row = payload.dict()
...
# Dual-route (see note on /api/health above) — POST the suggestion
# under both `/api/suggestions` and `/suggestions` so Vercel path
# forwarding (with or without /api/ prefix) routes correctly.
@app.post("/api/suggestions", status_code=201)
def post_suggestion(payload: SuggestionIn) -> dict[str, Any]:
    return _post_suggestion_impl(payload)


@app.post("/suggestions", status_code=201, include_in_schema=False)
def post_suggestion_no_prefix(payload: SuggestionIn) -> dict[str, Any]:
    return _post_suggestion_impl(payload)
    row = payload.dict()
    row["id"] = f"{payload.slug}-{int(datetime.now(timezone.utc).timestamp())}-{uuid.uuid4().hex[:6]}"
    row["received_at"] = datetime.now(timezone.utc).isoformat(timespec="seconds")
...
# Vercel's Python runtime reads `handler` from this module. Mangum
# bridges our ASGI app to Vercel's WSGI event surface so async
# FastAPI routes work in the serverless environment. lifespan="off"
# because there's no stable lifespan across cold starts and we don't
# want startup/shutdown hooks firing on every cold start.
